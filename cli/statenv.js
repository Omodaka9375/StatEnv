...
async function addAppCommand() {
  try {
    ...
    const origins = await question(
      'Allowed origins (comma-separated, e.g., https://myblog.com,http://localhost:3000): '
    );
    ...
    while (addMore) {
      ...
      if (httpMethod === 'GET') {
        const paramsInput = await question(
          'Allowed query params (comma-separated, e.g., q,lang) [q]: '
        );
        params = (paramsInput || 'q')
          .split(',')
          .map((p) => p.trim())
          .filter(Boolean);
      } else {
        const fieldsInput = await question(
          'Allowed body fields (comma-separated, e.g., event,data): '
        );
